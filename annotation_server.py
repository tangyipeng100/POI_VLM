...
DEFAULT_DATASET = ROOT / "dataset" / "night_150"
...
class AnnotationStore:
    ...
    def _load_samples(self) -> list[dict]:
        ...
        # Metadata-only rows remain useful for evaluation, but the local UI
        # should present only records with a bundled visual asset.
        return [row for row in rows if row.get("question_image") or row.get("front_poi_image")]
    ...
    def image_path(self, sample_id: str, kind: str) -> Path:
        ...
        value = sample.get(key) or sample.get("front_poi_image")
        if not value:
            raise KeyError(f"No public image for {sample_id}")
        candidate = Path(value)
        if not candidate.is_absolute():
            candidate = self.dataset_dir / candidate
        resolved = candidate.resolve()
        if self.dataset_dir.resolve() not in resolved.parents and resolved != self.dataset_dir.resolve():
